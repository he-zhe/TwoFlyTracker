function ff = show_fly_info_agg( movie,frame,posx,posy,agg_s,min_body_dist_s,speed_s,collisions,ROIs)
...
agg_insert = vision.TextInserter('%s', 'LocationSource', 'Input port','Color',  [205, 205, 36], 'FontSize', 24);
agg_pos = [ROIs(1)-ROIs(3)+2 15];

if agg_s(1,frame)==1
    strings_agg = uint8('Aggression');
    ff = step(agg_insert, ff, strings_agg, uint32(agg_pos));
